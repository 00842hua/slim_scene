...
DepthSepConv = namedtuple('DepthSepConv', ['kernel', 'stride', 'depth', 'se'])

# MOBILENETV1_CONV_DEFS specifies the MobileNet body
MOBILENETV1_CONV_DEFS = [
    Conv(kernel=[3, 3], stride=2, depth=32),
    DepthSepConv(kernel=[3, 3], stride=1, depth=64, se=0),
    DepthSepConv(kernel=[3, 3], stride=2, depth=128, se=1),
    DepthSepConv(kernel=[3, 3], stride=1, depth=128, se=1),
    DepthSepConv(kernel=[3, 3], stride=2, depth=256, se=0),
    DepthSepConv(kernel=[3, 3], stride=1, depth=256, se=0),
    DepthSepConv(kernel=[3, 3], stride=2, depth=512, se=1),
    DepthSepConv(kernel=[3, 3], stride=1, depth=512, se=1),
    DepthSepConv(kernel=[3, 3], stride=1, depth=512, se=1),
    DepthSepConv(kernel=[3, 3], stride=1, depth=512, se=1),
    DepthSepConv(kernel=[3, 3], stride=1, depth=512, se=1),
    DepthSepConv(kernel=[3, 3], stride=1, depth=512, se=1),
    DepthSepConv(kernel=[3, 3], stride=2, depth=1024, se=0),
    DepthSepConv(kernel=[3, 3], stride=1, depth=1024, se=0)
]
...
def _squeeze_excitation_layer(input, out_dim, ratio = 16, layer_name='se_block', reuse=None):
    with tf.variable_scope(layer_name, reuse=reuse):
        squeeze = tf.layers.average_pooling2d(input, pool_size=input.get_shape()[1:-1], strides=1,
                                    padding='valid', data_format='channels_last', name='global_avg')

        num_reduce_filters = max(1, int(out_dim / ratio))
        excitation = _fully_connected_layer(squeeze, units=num_reduce_filters, name=layer_name + '_excitation1',
                                            reuse=reuse)
      ...
      for i, conv_def in enumerate(conv_defs):
        ...
        if isinstance(conv_def, Conv):
          ...
          if conv_def.se:
            ...
            net = _squeeze_excitation_layer(net, out_dim=channel, layer_name=end_point)
