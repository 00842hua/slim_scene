...
def fairnas_c(inputs, num_classes, is_training=False, padding='SAME'):
    model_meta = [1, 0, 2, 0, 0, 0, 0, 0, 0, 3, 0, 0, 0, 0, 5, 5, 3, 3, 4]
    with slim.arg_scope(global_scope(is_training=is_training)):
        with tf.variable_scope("FairNas", values=[inputs]):
            end_points = {}
            net = conv2d_bn(inputs, 32, 3, 2, scope='stem', padding_type=padding)
            print("tf model padding:", padding)
            ...
            for index, id in enumerate(model_meta):
                ...
                net = res_block(net, expansion, channel, kernel_size, stride, scope="mb_%d" % index, padding_type=padding)
