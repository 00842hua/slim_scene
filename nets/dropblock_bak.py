...
class DropBlock(base._Layer):
    ...
    def call(self, inputs, training=False, scale=True, **kwargs):
        def drop():
            mask = self._create_mask(tf.shape(inputs))
            print("mask: ", mask)
            output = inputs * mask
            output = tf.cond(tf.constant(scale, dtype=tf.bool) if isinstance(scale, bool) else scale,
                             lambda: output * tf.to_float(tf.size(mask)) / tf.reduce_sum(mask), lambda: output)
            return output
    ...
    def _create_mask(self, input_shape):
        sampling_mask_shape = tf.stack([input_shape[0],
                                       ...
                                       self.channel])
        print("sampling_mask_shape: ", sampling_mask_shape)
