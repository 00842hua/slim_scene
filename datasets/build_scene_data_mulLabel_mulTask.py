...
tf.app.flags.DEFINE_string('mul_task_list_file',
                           None,
                           'mul_task_list_file')


tf.app.flags.DEFINE_string('mul_task_keyword_file',
                           None,
                           'mul_task_keyword_file')

tf.app.flags.DEFINE_integer('mul_task_num_classes', 3,
                            'Number of classes.')
...
mul_task_list = {}
mul_task_keyword = []
if FLAGS.mul_task_list_file:
    lines = tf.gfile.FastGFile(FLAGS.mul_task_list_file, 'r').readlines()
    for line in lines:
        sep = line.split("\t")
        if len(sep) == 2:
            curr_label = int(sep[1])
            mul_task_list[sep[0]] = curr_label

if FLAGS.mul_task_keyword_file:
    lines = tf.gfile.FastGFile(FLAGS.mul_task_keyword_file, 'r').readlines()
    for line in lines:
        sep = line.strip().split("\t")
        if len(sep) == 2:
            curr_label = int(sep[0])
            mul_task_keyword.append((sep[1], curr_label))

print("-------- len(mul_task_list): %s" % (len(mul_task_list)))
print("-------- len(mul_task_keyword): %s" % (len(mul_task_keyword)))
#for item in mul_task_keyword:
#    print("%s %d" % (item[0], item[1]))

def getMulTaskLabel(filename):
    label_ret = 0
    sep = filename.split("/")
    if sep[-1] in mul_task_list:
        label_ret = mul_task_list[sep[-1]]
        print("[match list] %s %s %d" % (filename, sep[-1], label_ret))
    else:
        for item in mul_task_keyword:
            if filename.find(item[0]) != -1:
                label_ret = item[1]
                #print("[match keyword] %s %s %d" % (filename, item[0], label_ret))
                break
    if label_ret >= FLAGS.mul_task_num_classes:
        label_ret = 0
    return label_ret
...
def _convert_to_example(image_buffer, label, height, width, label_2):
  ...
  example = tf.train.Example(features=tf.train.Features(feature={
      ...
      'image/label_2': _float_feature(label_2),
...
def _process_image_files_batch(coder, thread_index, ranges, name, filenames,
  ...
  for s in xrange(num_shards_per_batch):
    ...
    for i in files_in_shard:
      ...
      label_2 = getMulTaskLabel(filename)
      example = _convert_to_example(image_buffer, label, height, width, label_2)
