...
#wangfei, special loss weight for specail categories, such as cat/dog
tf.app.flags.DEFINE_string('special_labels_4_loss_weight', None,
                           'labels that need special loss weight.')
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    tf.logging.info("************************** FLAGS.special_labels_4_loss_weight: %s",
                    FLAGS.special_labels_4_loss_weight)

    # calc special labels loss weight
    loss_weight = tf.ones([FLAGS.batch_size, FLAGS.label_count], dtype=tf.float32)
    if FLAGS.special_labels_4_loss_weight:
        special_labels_4_loss_weight_list = []
        if FLAGS.special_labels_4_loss_weight is not None:
            special_labels_4_loss_weight_list = FLAGS.special_labels_4_loss_weight.split(",")
            special_labels_4_loss_weight_list = [int(item) for item in special_labels_4_loss_weight_list]
        tf.logging.info("************************** special_labels_4_loss_weight_list: %s",
                        special_labels_4_loss_weight_list)
        special_labels_4_loss_weight_list_indices = [[0, x] for x in special_labels_4_loss_weight_list]
        special_labels_4_loss_weight_list_values = [1.0 for _ in special_labels_4_loss_weight_list]
        tf.logging.info("************************** special_labels_4_loss_weight_list_indices: %s",
                        special_labels_4_loss_weight_list_indices)
        special_labels_4_loss_weight_list_add_dense_tensor = tf.SparseTensor(special_labels_4_loss_weight_list_indices,
                                                                       values=special_labels_4_loss_weight_list_values,
                                                                       dense_shape=[1, FLAGS.label_count])
        special_labels_4_loss_weight_list_add_tensor = tf.sparse.to_dense(special_labels_4_loss_weight_list_add_dense_tensor)
        tf.logging.info(special_labels_4_loss_weight_list_add_tensor)
        special_labels_4_loss_weight_list_add_tensor = tf.tile(special_labels_4_loss_weight_list_add_tensor, [FLAGS.batch_size, 1])
        tf.logging.info(special_labels_4_loss_weight_list_add_tensor)
        # sess = tf.Session()
        # tf.logging.info(sess.run(special_labels_4_loss_weight_list_add_tensor))
        loss_weight = loss_weight + special_labels_4_loss_weight_list_add_tensor
    tf.logging.info("************************** loss_weight.shape: %s", loss_weight.shape)
    tf.logging.info(loss_weight)
    sess = tf.Session()
    tf.logging.info(sess.run(loss_weight))
    ...
    with tf.device(deploy_config.inputs_device()):
      ...
      image = image_preprocessing_fn(image, train_image_size, train_image_size, \
                                     crop_area_min=FLAGS.crop_area_min)
    ...
    def clone_fn(batch_queue):
      ...
      if 'AuxLogits' in end_points:
        ...
        tf.losses.sigmoid_cross_entropy(
            labels, end_points['AuxLogits'],
            label_smoothing=FLAGS.label_smoothing, weights=loss_weight*tf.convert_to_tensor(0.4),
            scope='aux_loss')
      #slim.losses.softmax_cross_entropy(
      ...
      tf.losses.sigmoid_cross_entropy(
          labels, logits, label_smoothing=FLAGS.label_smoothing, weights=loss_weight*tf.convert_to_tensor(1.0))
      return end_points
