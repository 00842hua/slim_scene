...
def conv2d_bn(inputs, out_channel, kernel_size, stride, activation_fn=tf.nn.relu6, scope=None):
    with tf.variable_scope(scope):
        if stride == 1:
            ...
            inputs = tf.pad(inputs, paddings=[[0, 0], [padding, padding], [padding, padding], [0, 0]])
            net = slim.conv2d(inputs,
                              num_outputs=out_channel,
                              kernel_size=kernel_size,
                              stride=stride,
...
def res_block(inputs, expansion_ratio, out_channel, kernel_size, stride, activation_fn=tf.nn.relu6, scope=None):
    with tf.variable_scope(scope):
