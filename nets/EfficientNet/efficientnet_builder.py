...
def get_model_params(model_name, override_params, num_classes=1001):
  """Get the block args and global params for a given model."""
  if model_name.startswith('efficientnet'):
    width_coefficient, depth_coefficient, _, dropout_rate = (
        efficientnet_params(model_name))
    blocks_args, global_params = efficientnet(
        width_coefficient, depth_coefficient, dropout_rate, num_classes=num_classes)
  else:
...
def build_model(images,
  ...
  with tf.variable_scope(model_name+"/Logits"):
    logits = tf.identity(logits, 'logits')
    model.endpoints['Predictions_softmax'] = tf.nn.softmax(logits, name='Predictions_softmax')
    model.endpoints['Predictions_sigmoid'] = tf.nn.sigmoid(logits, name='Predictions_sigmoid')
...
efficient_net_b0 = wrapped_partial(build_model, model_name="efficientnet-b0")
