...
do
    ...
    do
        ...
        pb_path=${dir}/model.ckpt-${one_ckpt_step}_quant.pb
        score_file=${dir}/result_score_${one_ckpt_step}_quant.txt
        echo "-----------------------------------------------------------------------------" >&2
        ...
        then
            ...
            CUDA_VISIBLE_DEVICES=""  python export_inception_model_xiaomi.py --check_point=${ckpt_path} --graph_path=${pb_path} --labels_number=${LABEL_NUM} --net_type=${net_type} --image_size=${img_size} --quantize=True
            
            gpu=`nvidia-smi  | grep Default | awk '{print NR-1"\t"$9}' | sort -k2 -n | head -n1 | awk '{print $1}'`
            echo "---------------------gpu: " ${gpu}
            CUDA_VISIBLE_DEVICES="${gpu}"   python -u compute_scores.py \
                --pb_path=${pb_path} \
