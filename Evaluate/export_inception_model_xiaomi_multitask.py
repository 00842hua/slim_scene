...
tf.app.flags.DEFINE_integer('labels_number_task1', None, 'input class_num ')
tf.app.flags.DEFINE_integer('labels_number_task2', None, 'input class_num ')
...
# add @20200921 for multi-task heatmap
tf.app.flags.DEFINE_integer('heatmap_class_index', None, 'class index for heatmap calc.')
...
if __name__ == "__main__":
    ...
    if FLAGS.heatmap_class_index:
        network_fn = nets_factory.get_network_fn(
            net_type,
            num_classes=FLAGS.labels_number_task1,
            num_classes_task2=FLAGS.labels_number_task2,
            heatmap_class_index=FLAGS.heatmap_class_index,
            is_training=False)
    else:
        network_fn = nets_factory.get_network_fn(
            net_type,
...
#     stats_graph(sess.graph)
    ...
    if FLAGS.net_type == 'inception_resnet_v2' or FLAGS.net_type == 'inception_resnet_v2_dropblock':
    ...
    elif FLAGS.net_type == "mobilenet_v2_qc":
        output_node_names = "MobilenetV2/Predictions_sigmoid,MobilenetV2/Predictions_softmax," \
                            'MobilenetV2/Predictions_softmax_task2,MobilenetV2/Predictions_sigmoid_task2,' \
                            'MobilenetV2/Predictions_softmax_concat,MobilenetV2/Predictions_sigmoid_concat'
    elif FLAGS.net_type.startswith("mobilenet_v2_qc_multibranch"):
        output_node_names = "MobilenetV2/Predictions_sigmoid,MobilenetV2/Predictions_softmax," \
                            'MobilenetV2_branch2/Predictions_sigmoid,MobilenetV2_branch2/Predictions_softmax,' \
                            'Predictions_softmax_concat,Predictions_sigmoid_concat'
        ...
        output_node_names = "MobilenetV2/Predictions"
    elif FLAGS.net_type == 'mobilenet_v2_050':
        output_node_names = "MobilenetV2/Predictions/Reshape_1,MobilenetV2/Predictions_sigmoid,MobilenetV2/Predictions_softmax"
    ...
    elif FLAGS.net_type == 'mnasnet_a1':
        output_node_names = 'Mnasnet/Predictions_softmax,Mnasnet/Predictions_sigmoid,' \
                       'Mnasnet/Predictions_softmax_task2,Mnasnet/Predictions_sigmoid_task2,' \
                       'Mnasnet/Predictions_softmax_concat,Mnasnet/Predictions_sigmoid_concat'
    elif FLAGS.net_type == 'mnasnet_b1':
        output_node_names = 'Mnasnet/Predictions_softmax,Mnasnet/Predictions_sigmoid'
    else:
...
#                              clear_devices, "")
    ...
    try:
        ...
        print('Error in freeze graph. Try another way...')
        export_var_dir = './tmpvar/'
        builder = tf.saved_model.builder.SavedModelBuilder(export_var_dir)
        builder.add_meta_graph_and_variables(sess, [tf.saved_model.tag_constants.SERVING],
                                             strip_default_attrs=True)
        builder.save()
        freeze_graph.freeze_graph_with_def_protos(sess.graph_def, input_saver_def_path,
                                                  input_checkpoint_path, output_node_names,
                                                  restore_op_name, filename_tensor_name,
                                                  output_graph_path, clear_devices, "",
                                                  input_saved_model_dir=export_var_dir,
                                                  saved_model_tags=[tf.saved_model.tag_constants.SERVING]
                                                  )
        import shutil
        shutil.rmtree(export_var_dir)
