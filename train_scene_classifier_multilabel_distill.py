...
tf.app.flags.DEFINE_integer(
    'train_image_size_teacher_L', None, 'Train image size')

tf.app.flags.DEFINE_integer(
    'train_image_size_student_S', None, 'Train image size')
...
def _get_init_fn():
  ...
  for var in slim.get_model_variables():
    if var.op.name.startswith("Distill/"):
        distill_variables_to_restore[var.name.lstrip("Distill/").split(":")[0]] = var
    else:
      excluded = False
...
def main(_):
  ...
  if FLAGS.distill_model_name == "inception_resnet_v2":
    distill_inres_scope = "Distill/InceptionResnetV2"
  else:
    raise ValueError('FLAGS.distill_model_name %s Invalid' % FLAGS.distill_model_name)
  ...
  with tf.Graph().as_default():
    ...
    # Select the network #
    ######################
    distill_network_fn = None
    if FLAGS.distill_model_name and FLAGS.distill_model_ckpt:
        distill_network_fn = nets_factory.get_network_fn(
            FLAGS.distill_model_name,
    ...
    with tf.device(deploy_config.inputs_device()):
      ...
      imageL, imageS = image_preprocessing_fn(image, train_image_size_teacher_L, \
                                        train_image_size_student_S, \
    ...
    def clone_fn(batch_queue):
      ...
      distill_logits, _ = distill_network_fn(imagesL, scope=distill_inres_scope)
      distill_prediction = tf.nn.sigmoid(distill_logits)
      tf.logging.info("************************** distill_prediction: %s", distill_prediction)
      ...
      if 'AuxLogits' in end_points:
        ...
        tf.losses.sigmoid_cross_entropy(
            ...
            scope='aux_loss')
        tf.losses.sigmoid_cross_entropy(
            distill_prediction, end_points['AuxLogits'],
            label_smoothing=FLAGS.label_smoothing, weights=0.4,
            scope='aux_loss')
      tf.losses.sigmoid_cross_entropy(
