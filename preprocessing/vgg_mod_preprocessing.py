...
from preprocessing import vgg_preprocessing_400
...
def preprocess_for_train(image, height, width, bbox,
    ...
    with tf.name_scope(scope, 'distort_image', [image, height, width, bbox]):
        ...
        distorted_image, distorted_bbox = \
            vgg_preprocessing.distorted_bounding_box_crop(image, bbox,
        ...
        distorted_image = vgg_preprocessing.apply_with_random_selector(
            distorted_image,
        ...
        if distort_color:
            distorted_image = vgg_preprocessing.apply_with_random_selector(
                distorted_image,
                lambda x, ordering: vgg_preprocessing.distort_color(x, ordering, fast_mode),
                num_cases=4)
        '''
        if scope==None:
...
def preprocess_image_base(image, height, width,
    ...
    if is_training:
        ...
        return vgg_preprocessing.preprocess_for_eval(image, height, width)
...
class inception_preprocess_no_crop:
    ...
    def preprocess_image(image, height, width,
                         ...
                         scope=None):
        if is_training:
            return vgg_preprocessing.preprocess_for_train(image, height, width, bbox, fast_mode, scope=scope)
        else:
            return vgg_preprocessing.preprocess_for_eval(image, height, width, central_fraction=None)
...
class inception_preprocess_edge_no_crop:
    ...
    def preprocess_image(image, height, width,
                         ...
                         scope=None):
        if is_training:
            image = vgg_preprocessing.preprocess_for_train(image, height, width, bbox, fast_mode, scope=scope)
        else:
            image = vgg_preprocessing.preprocess_for_eval(image, height, width, central_fraction=None)
...
class inception_preprocess_resize_area:
    ...
    def preprocess_image(image, height, width,
                         ...
                         scope=None):
        if is_training:
            return vgg_preprocessing.preprocess_for_train(image, height, width, bbox, True, scope=scope)
        else:
...
class inception_preprocess_not_fast:
    ...
    def preprocess_image(image, height, width,
                         ...
                         scope=None):
        if is_training:
            return vgg_preprocessing.preprocess_for_train(image, height, width, bbox,
...
class inception_preprocess_all_400:
    @staticmethod
    def preprocess_image(image, height, width,
                         is_training=False,
                         bbox=None,
        ...
        if is_training:
            ...
            image = vgg_preprocessing_400.preprocess_for_train(image, height, width)
            ...
            return vgg_preprocessing_400.preprocess_for_eval(image, height, width)
