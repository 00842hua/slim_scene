...
def se_block(input_tensor, input_channel, output_channel,
             se_ratio=0.25,
             scope=None):
    with tf.variable_scope(scope, default_name='se_block') as s, \
        ...
        num_reduced_channel = max(1, int(int(input_channel) * se_ratio))
        num_channel = output_channel
        ...
        nb, ih, iw, nc = input_tensor.get_shape().as_list()
        _, eh, ew, _ = excitation_expand.get_shape().as_list()
        filter = tf.transpose(excitation_expand, [1, 2, 0, 3])
        filter = tf.reshape(filter, [eh, ew, nb * nc, 1])

        input_r = tf.transpose(input_tensor, [1, 2, 0, 3])
        input_r = tf.reshape(input_r, [1, ih, iw, nb * nc])

        out = tf.nn.depthwise_conv2d(input_r, filter, strides=[1, 1, 1, 1], padding='VALID')
        out = tf.reshape(out, [ih, iw, nb, nc])
    ...
    with tf.variable_scope(scope, default_name='sep_conv') as s, \
        ...
        if stride == 0:
            net = slim.separable_conv2d(inputs=input_tensor,
                                        ...
                                        activation_fn=tf.nn.relu,  # tf.nn.relu6,
                                        padding=padding,
                                        scope='depthwise')
        else:
            pad = kernel_size[0] // 2
            net = tf.pad(input_tensor, paddings=[[0, 0], [pad, pad], [pad, pad], [0, 0]])
...
def mb_conv(input_tensor,
    ...
    with tf.variable_scope(scope, default_name='mb_conv') as s, \
        ...
        # add@20190830 for quantize training
        input_tensor = tf.identity(input_tensor, 'input')
        net = input_tensor
        input_channel = input_tensor.shape.as_list()[-1]  # TODO fix the following

        net = slim.conv2d(inputs=net,
                          num_outputs=input_channel * expand_factor,  # num_outputs * expand_factor,
                          kernel_size=[1, 1],
                          padding=padding,
                          normalizer_fn=slim.batch_norm,
                          activation_fn=tf.nn.relu,  # tf.nn.relu6,
        ...
        if stride == 0:
            ...
            pad = kernel_size[0] // 2
            net = tf.pad(net, paddings=[[0, 0], [pad, pad], [pad, pad], [0, 0]])
        ...
        # TODO fix, moving the se block above
        if se_ratio > 0 and se_ratio <= 1:
            # net = se_block(net, se_ratio)
            net = se_block(net, input_channel, input_channel * expand_factor, se_ratio)
...
MNASNET_A1_DEF = dict(
    defaults={
        ...
        (slim.conv2d, slim.fully_connected, slim.separable_conv2d): {
            'normalizer_fn': slim.batch_norm, 'activation_fn': tf.nn.relu,  # tf.nn.relu6
        },
        ...
        for i, opdef in enumerate(conv_defs['spec']):
            ...
            if use_explicit_padding:
                if 'kernel_size' in params:
                    if opdef.op == slim.conv2d and params['stride'] == 2:
...
mnasnet_a1.default_image_size = 224
mnasnet_b1.default_image_size = 224
