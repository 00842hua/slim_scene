...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    # Select the dataset #
    ######################
    dataset_name = "pair_rank_dataset"
    tf.logging.info("************************** dataset_name:     %s", dataset_name)
    dataset = dataset_factory.get_dataset(
        dataset_name, FLAGS.dataset_split_name, FLAGS.dataset_dir,
    ...
    # preprocessing_name = "inception_notcrop"
    image_preprocessing_fn = preprocessing_factory.get_preprocessing(
        preprocessing_name,
        is_training=True)

    tf.logging.info("************************** preprocessing_name:     %s", preprocessing_name)
    ...
    with tf.device(deploy_config.inputs_device()):
      ...
      [image1, image2] = provider.get(['image1', 'image2'])
      tf.logging.info("************************** image1.shape:     %s", image1.shape)

      train_image_size = FLAGS.train_image_size or network_fn.default_image_size

      image1 = image_preprocessing_fn(image1, train_image_size, train_image_size)
      image2 = image_preprocessing_fn(image2, train_image_size, train_image_size)
      tf.logging.info("************************** image1.shape:     %s", image1.shape)

      images1, images2 = tf.train.batch(
          [image1, image2],
          batch_size=FLAGS.batch_size,
          num_threads=FLAGS.num_preprocessing_threads,
          capacity=5 * FLAGS.batch_size)
      batch_queue = slim.prefetch_queue.prefetch_queue(
          [images1, images2], capacity=2 * deploy_config.num_clones)
    ...
    def clone_fn(batch_queue):
      ...
      images1, images2 = batch_queue.dequeue()
      tf.logging.info("************************** images1:     %s", images1)
      tf.logging.info("************************** images2:     %s", images2)

      # logits1, end_points = network_fn(images1)
      # logits2, end_points = network_fn(images2, reuse=True)

      images_merge = tf.concat([images1, images2], axis=0)
      tf.logging.info("************************** images_merge:     %s", images_merge)
      logits_merge, end_points = network_fn(images_merge)
      tf.logging.info("************************** logits_merge:     %s", logits_merge)
      logits1, logits2  = tf.split(logits_merge, num_or_size_splits=2, axis=0)

      tf.logging.info("************************** logits1:     %s", logits1)
      tf.logging.info("************************** logits2:     %s", logits2)

      diff = logits2 - logits1 + 1
      tf.logging.info("************************** diff:     %s", diff)
      zeros = tf.zeros_like(diff)
      diff = tf.where(diff < 0, x=zeros, y=diff)
      rank_loss = tf.reduce_mean(diff)
      # rank_loss = tf.reduce_sum(diff)
      tf.logging.info("************************** rank_loss:     %s", rank_loss)
      tf.losses.add_loss(rank_loss)
      tf.logging.info("************************** tf.losses:     %s", tf.losses)
      return end_points
