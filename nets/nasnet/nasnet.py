...
def _update_hparams(hparams, is_training):
  ...
  if not is_training:
    ...
    if hasattr(hparams,'set_hparam'):
      hparams.set_hparam('drop_path_keep_prob', 1.0)
    else:
      setattr(hparams,'drop_path_keep_prob', 1.0)
...
def build_nasnet_large_with_scope(images, num_classes,
                       is_training=True,
                       final_endpoint=None,
                       config=None,
                       scope='NasnetLarge'):
      ...
      with arg_scope([slim.dropout, nasnet_utils.drop_path, slim.batch_norm],
        ...
        with arg_scope([slim.avg_pool2d,
          ...
          return _build_nasnet_base(images,
                                    ...
                                    stem_type='imagenet',
                                    final_endpoint=final_endpoint)
build_nasnet_large_with_scope.default_image_size = 331
...
def _build_nasnet_base(images,
  ...
  with tf.variable_scope('final_layer'):
    ...
    predictions_sigmoid = tf.nn.sigmoid(logits, name='predictions_sigmoid')
    if add_and_check_endpoint('Predictions_sigmoid', predictions_sigmoid):
      return net, end_points
