...
def mobilenet(inputs,
  ...
  """
  num_classes_task2 = mobilenet_args.pop('num_classes_task2', None)
  ...
  with tf.variable_scope(scope, 'Mobilenet', reuse=reuse) as scope:
    ...
    if num_classes_task2:
      with tf.variable_scope('Logits'):
        logits_task2 = slim.conv2d(
          net,
          num_classes_task2, [1, 1],
          activation_fn=None,
          normalizer_fn=None,
          biases_initializer=tf.zeros_initializer(),
          scope='Conv2d_1c_1x1_task2')
        logits_task2 = tf.squeeze(logits_task2, [1, 2])
        logits_task2 = tf.identity(logits_task2, name='output_task2')

        end_points['Logits_task2'] = logits_task2
        end_points['Predictions_softmax_task2'] = tf.nn.softmax(logits_task2, name='Predictions_softmax_task2')
        end_points['Predictions_sigmoid_task2'] = tf.nn.sigmoid(logits_task2, name='Predictions_sigmoid_task2')
        end_points['Predictions_softmax_concat'] = tf.concat([end_points['Predictions_softmax'],
                                                             end_points['Predictions_softmax_task2']],
                                                             axis=-1,
                                                             name='Predictions_softmax_concat')
        end_points['Predictions_sigmoid_concat'] = tf.concat([end_points['Predictions_sigmoid'],
                                                             end_points['Predictions_sigmoid_task2']],
                                                             axis=-1,
...
def training_scope(is_training=True,
  ...
  batch_norm_params = {
      'decay': bn_decay,
      'is_training': False if freeze_bn else is_training,  # for partial bn in tsm
      'trainable': False if freeze_bn else True  # for partial bn in tsm
  }
  print('bn is training: {}, trainable: {}'.format(batch_norm_params['is_training'], batch_norm_params['trainable']))
  if stddev < 0:
  ...
  with slim.arg_scope(
      ...
      slim.arg_scope([slim.separable_conv2d], weights_regularizer=depthwise_regularizer) as s:
    return s