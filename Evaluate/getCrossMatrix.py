...
if len(sys.argv) >= 6:
    ...
    for k in label_map_dict:
        ...
        if k not in labeldict_name2idstr:
            continue
...
for i in range(clsnum):
    line = labeldict[i][:10].rjust(11) + ' R = ' + '{:.8f}'.format(r_count[i]) + ' P = ' + '{:.8f}'.format(p_count[i]) \
            + ' P_AllDim = ' + '{:.8f}'.format(p_alldim_count[i])+ ' ap = ' + '{:.8f}'.format(p_list[i]) \
            + '\tTotal = ' + '{:03d}'.format(tp_count[i] + fp_count[i]) \
			+ '\tthres = ' + '{:.3f}'.format(thresholds[labeldict[i]]) + '\n'
